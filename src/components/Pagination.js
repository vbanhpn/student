...
export default function Pagination() {
  ...
  const handleMoveToPreviousPage = () => {
    if (currentPage - 1 > 0) {
      ...
      if (currentPage === pagesNumber[0]) dispatch(decreasePageNumber());
    } else return;
  };
  ...
  const handleMoveToNextPage = () => {
    if (currentPage + 1 <= total) {
      ...
      if (currentPage === pagesNumber[4]) dispatch(increasePageNumber());
    } else return;
  };
  ...
  return (
    <div className={style.pages}>
      ...
      {pagesNumber.map((number) => (
        <PageButton key={number} number={number} currentPage={currentPage} />
      ))}
